...
if ( ! function_exists( 'miraforever_entry_footer' ) ) :
/**
 ...
 * @return bool
 */
function miraforever_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'miraforever_categories' ) ) ) {
		...
		set_transient( 'miraforever_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so miraforever_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so miraforever_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in miraforever_categorized_blog.
 */
function miraforever_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'miraforever_categories' );
}
add_action( 'edit_category', 'miraforever_category_transient_flusher' );
add_action( 'save_post',     'miraforever_category_transient_flusher' );
